...
from simplenmt.constants import BOS_ID, EOS_ID, PAD_ID
...
def greedy_search(model, src_tokens, max_seq_len=MAX_SEQ_LEN, bos=BOS_ID, eos=EOS_ID, src_pdx=PAD_ID, tgt_pdx=PAD_ID):
...
def beam_search(model, src_tokens, beam_size, length_penalty, max_seq_len=MAX_SEQ_LEN, bos=BOS_ID, eos=EOS_ID, src_pdx=PAD_ID, tgt_pdx=PAD_ID):
    # batch size
    batch_size = len(src_tokens)
    ...
    while cur_len < max_seq_len:
        ...
        for sent_id in range(batch_size):
            ...
            for idx, value in zip(next_words[sent_id], next_scores[sent_id]):
                ...
                beam_id = torch.div(idx, n_tgt_words, rounding_mode='floor')
